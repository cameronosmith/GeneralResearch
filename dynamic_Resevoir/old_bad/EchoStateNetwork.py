...
update_info_interval  = 100 #just for printing output not maths 
...
class EchoStateNetwork : 
    ...
    #@param res_class   : the class of the resevoir to use
    def __init__( self, input_size, output_size, res_class ):

        self.input_size  = input_size
        self.output_size = output_size

        #init our random weights and resevoir
        self.weights_in  = np.random.rand ( 3, input_size ) - range_shift 
        self.resevoir    = res_class( res_size, connectivity, leak_rate )
    ...
    def train ( self, train_input, train_output ):
        ...
        train_input = ChaoticResevoir.scale_input( train_input )

        #holds our states at every time step (used to construct outputs weights)
        states_matrix = np.zeros( (self.resevoir.res_size, len(train_input)) )
    ...
    def test ( self, test_input, test_truth ):
        ...
        test_input = ChaoticResevoir.scale_input( test_input )

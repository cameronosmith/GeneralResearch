...
def lorenz_step ( x,y,z, sigma=10,beta=8/3,rho=28, time_step=.1 ) :
    x,y,z=x[0],y[0],z[0]
    ...
    new_point 	= [round(sum(i),5) for i in zip(deltas, [x,y,z])]
...
class Resevoir : 
    ...
    class Resevoir_Node : 
        ...
        def run ( self, weighted_data, weights_row ):
            if np.array_equal( self.state , np.zeros( 3 ) ):
                self.state = weighted_data 
            
            hidden_update   = lorenz_step( self.state )
            #hidden_update   = lorenz_step( *hidden_update )
    ...
    def get_hidden_states( self ):
        ...
        formatted   = np.array( raw_states ).reshape( (self.res_size*3, 1) )

...
#@param curr_state 	: current x,y,z state as list
#@return 			: the next state of the system (x,y,z)
rossler_step <- function ( curr_state, time_step ) { 
    a <- .2; b<-.2; c<-14;
	x <- curr_state[1]; y <- curr_state[2]; z<- curr_state[3];
	dx <- -y-z; dy <- x+a*y - y; dz <- beta+z*(x-c);
	diffs <- c(dx,dy,dz)
	return (diffs*time_step+curr_state)
}
...
#function to get the output of the series one step ahead
#@return : the dataframe with the lorenz data
sim_rossler <- function( start_state = c(12,13,14), time_step = .05, num_steps = 2000) {
	rossler_data		 	<- matrix(0, ncol = length(start_state), nrow = num_steps)
	colnames(rossler_data)	<- c("x","y","z")
	lorenz_data[1,] 		<- start_state
	for ( row_idx in 2:nrow(rossler_data)){
		prev_state 				<- rossler_data[row_idx-1,]	
		new_state				<- rossler_step( prev_state, time_step )
		rossler_data[row_idx,]	<- new_state
	}
	return ( as.data.frame(rossler_data) )
}
